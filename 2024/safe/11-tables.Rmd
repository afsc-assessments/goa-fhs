...
```{r tab1}
catches <-  read.csv(here::here(this_year,'data','output','2024-06-06-catch_observed.csv')) %>% filter(year >=2000)
...
flextable::regulartable(catches) %>%  
  ...
  flextable::set_header_labels(year = 'Year', TTONS = 'Total Catch (t)', WGOA = 'Western Gulf', CGOA= 'Central Gulf',EGOA = 'Eastern Gulf') %>%
  flextable::colformat_double(j = 2:4, big.mark = ',', digits = 0) %>%
  flextable::bold(i = 23:25,j = 1) %>%
...
output$biomass_by_strata %>%
  ...
  dplyr::filter(!is.na(obs) & year >=1990) %>% ## drop interpolated years and pre 1990s
