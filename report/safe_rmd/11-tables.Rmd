...
bind_rows(  
  base_model$derived_quants[grep('SSB_', base_model$derived_quants$Label),c('Label','Value','StdDev')] %>%
  mutate(Year = as.numeric(gsub('SSB_',"",Label)), Value = round(Value,4)) %>%
  select(Year, 'Spawning Biomass' = Value, SD = StdDev) %>%
    filter(!is.na(Year)) %>% reshape2::melt(id = c('Year','SD')),
...
bigts$value[bigts$Model == 'Model 17.1a (2022)' & bigts$Year>2021] <- ""
bigts$SD[bigts$Model == 'Model 17.1a (2022)' & bigts$Year>2021] <- ""

## compound column
bigts$value_SD <- paste0(prettyNum(bigts$value,big.mark = ",") ," (",prettyNum(bigts$SD,big.mark = ","),")")
bigts$Year <- as.character(bigts$Year)

## add in projected stuff
# bigts[nrow(bigts+1),] <- c('Model 17.1a (2022)', '2022', variabl)
bigts %>%
  arrange(variable) %>%
...
#   # color(part = "footer", color = "#800000") %>%
#   flextable::bold( bold = TRUE, part="header") %>% 
#   flextable::align(i = ~ !is.na(Model), align = "center") %>% 
#   flextable::bold(i = ~ !is.na(Model)) %>%  
# 
