...
Since reliable estimates of the `r year+1` spawning biomass (B), $B_{40\%}$, $F_{40\%}$, and $F_{35\%}$ exist and B>$B_{40\%}$, the `r species` reference fishing mortality is defined in Tier 3a.
...
Amendment 56 to the GOA Groundfish Fishery Management Plan defines the "overfishing level" (OFL), the fishing mortality rate used to set OFL ($F_{OFL}$), the maximum permissible ABC, and the fishing mortality rate used to set the maximum permissible ABC.
...
The `r year` estimates of these reference points are shown below. Values are from the projection model for `r base_model_name` and should not be compare directly to outputs from Stock Synthesis forecast outputs.
...
Applying these fishing mortality rates for `r year`, yields an ABC of `r safe[12,4]` t and an OFL of `r safe[11,4]` t.
...
The spawning stock biomass has been above target for the entire time period for which there are data (Figure \@ref(fig:BFtrajectory)). The estimated age 3+ biomass has increased steadily since 2010, coincident with more positive than negative recruitment deviations in the last 10 years (Figure \@ref(fig:rects)). Since we have no increased concerns we set the concern level to 1.
...
This fishery has consistently caught only a small fraction of the ABC for the last 10 years (Figure \@ref(fig:catchvsTAC)). We did not examine CPUE trends nor spatial patterns of fishing. There are no changes in the duration of fishing openings. Altogether, we see no cause for concern and give this consideration a level 1 as well. 
