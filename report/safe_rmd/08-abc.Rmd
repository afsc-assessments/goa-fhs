...
The reference fishing mortality rate for flathead sole is determined by the amount of reliable population information available (Amendment 56 of the Fishery Management Plan for the groundfish fishery of the Bering Sea/Aleutian Islands).
Estimates of $F_{40%}$, $F_{35%}$, and $SPR_{40%}$ were obtained from a spawner-per recruit analysis.
Assuming that the average recruitment from the 1983-2012 year classes estimated in this assessment represents a reliable estimate of equilibrium recruitment, then an estimate of $B_{40%}$ is calculated as the product of SPR40% times the equilibrium number of recruits.
Since reliable estimates of the `r year+1` spawning biomass (B), $B_{40%}$, $F_{40%}$, and $F_{35%}$ exist and B\>$B_{40%}$, the flathead sole reference fishing mortality is defined in Tier 3a.
For this tier, $F_{ABC}$ is constrained to be ≤ $F_{40%}$, and FOFL is defined to be $F_{35%}$.
...
| Quantity                      | Value |
|-------------------------------|-------|
| Stock Spawning Biomass (2023) |    `r 2+2`   |
|                               |       |
|                               |       |
|                               |       |
|                               |       |
...
: Unnumbered Table. Harvest recommendations from Model 17.1a. Values are from the projection model.
