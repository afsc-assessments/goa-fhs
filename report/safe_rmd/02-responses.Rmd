...
We did not have time to conduct more than one sensitivity analysis for this topic (e.g., removing 1980s survey data, separating early 1990s survey data, and combinations thereof). Instead, we conducted one sensitivity analysis where all survey biomass, length composition, and conditional age-at-length data through 1993 were truncated (removed) due to the differences in survey design. Do note that the base model here, and previous models before it, already disabled CAAL data from the survey before 1993. Given the structure of the current model, we did not see strong differences in terminal year biomass, recruitment deviations, or survey fits between these two models. It is likely this was more of an issue for other species considered in the CIE report. For thoroughness, if future cycles retain these early survey years and include the calculation or estimation of catchability $q$ different from 1.0, it would be worth considering introducing a time-block on catchability and/or selectivity. 
