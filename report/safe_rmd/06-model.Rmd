...
mean proportions-at-age and proportions-at-length (Francis, 2011). To
...
In the 2015 assessment and the 2017 assessment, the method described in
...
Pennington 1994). The McAllister-Ianelli method for weighting among data
sources was used in the 2015 and 2017 assessment (McAllister and Ianelli
1997).
...
The 2015 and 2017 stock assessments incorporated ageing error by using
an existing ageing error matrix for BSAI flathead sole, for which error
increased lienarly from age 0 to a maximum at age 16. External to this
assessment, we revisited the ageing error data for GOA flathead sole
...
and updated ageing error matrices is shown in FIGURE XX. BSAI and GOA
flathead sole are aged by the same individuals using the same techniques
and ageing error is expected to be very similar.

Data, code, and additional figures associated with the updated aging
error matrix can be found at XX. An examination of this years base model
(model XX) using the old ageing error matrix XX did not have a
...
recruits (1984-2012) such that the vector of recruits for each period
...
The breakpoints for the bias adjustment ramp were as follows: a linear
increase from zero to a plateau of from
`r m0_8$breakpoints_for_bias_adjustment_ramp[5]` from
`paste0(range(round(as.numeric(m0_8$breakpoints_for_bias_adjustment_ramp))[1:2]), collapse = '-')`,
after corresponding to the onset of regular compositional data; constant
bias adjustment from
`r paste0(range(round(as.numeric(m0_8$breakpoints_for_bias_adjustment_ramp))[2:3]), collapse = '-')`,
and a decline from the plateau to zero between years
`r paste0(range(round(as.numeric(m0_8$breakpoints_for_bias_adjustment_ramp))[3:4]), collapse = '-')`.

## Description of Alternative Models

The model presented herein follows the same model structure and data
input types as the most recent (2017) accepted model for GOA flathead
...
sources to reflect the current information provided by AKFIN. Following
the recent CIE review, we present a handful of model explorations as
sensitivities in section XX. We also present the previously accepted
...
comparison in FIGURE XX. No new model structures are recommended this
year.
...
structure as model 2017.

### Natural Mortality

The natural mortality rates were fixed at 0.2 for both sexes, as was
done for previous assessments. We explore a sensitivity run where
natural mortality is estimated [as an offset/for each sex] in section
XX.

### Weight at Length

The following weight-length relationship used in the previous assessment
(McGilliard et al. 2013) is used in the current assessment: , where and
, length (L) was measured in centimeters and weight (w) was measured in
kilograms.
...
`r subset(m0_8$parameters, Label == "Mat_slope_Fem_GP_1")[,'Value']` and
age at 50% maturity `r`r subset(m0_8\$parameters, Label ==
"Mat50%\_Fem_GP_1")[,'Value']\`. These were obtained by histological
analysis of 180 samples of GOA flathead sole ovaries collected in the
central Gulf of Alaska from January 1999 (Stark 2004), and are the same
as was used in the 2017 assessment.

### Standard Deviation of log Recruitment ( $\sigma_R$)

Variability of the recruitment deviations that were estimated in
previous flathead sole assessments was approximately $\sigma_R=0.6$,
which is fixed within the current assessment.

### Survey Catchability

The survey catchability parameter $q$ was fixed at 1.0, as for previous
flathead sole assessments. We explore a sensitivity run where survey
catchability is allowed to be calculated analytically in section XX.
...
were estimated (see Section XX). A 1:1 sex ratio is assumed.
