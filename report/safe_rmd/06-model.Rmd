...
selectivity parameters by fleet and sex are presented in Table \@ref(tab:tab6selex).
...
An examination of `r base_model_name` using the previous ageing error matrix did not have a
...
constraint across all recruitment deviations. The main period ends in `r `max(subset(rd_breaks, era == 'Main')$yr` as the age at recruitment used for projections beginning in 2023 is assumed to be three years; recruitment deviations are therefore set to zero for 2021 and 2022.
...
We also present the previously accepted model in its original form (in Synthesis version 3.24) for comparison in Figures \@ref(fig:ssbcompare) to \@ref(fig:selexcompare). No new model structures are recommended this year.
...
log-scale recruitment deviations; 3) Sex-specific parameters corresponding to von-Bertalanffygrowth, maturity, and natural morality (`r length(grep('GP',base_model$parameters$Label[base_model$parameters$Phase >0]))`) , 4) `r length(grep('Sel|DblN',base_model$parameters$Label[base_model$parameters$Phase >0]))` selectivity parameters in total
for the fishery fleet and and survey (see Table @\ref(tab:tab6selex)).
