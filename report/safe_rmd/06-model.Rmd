...
assessment, we revisited the ageing error data for GOA  `r species` 
...
error matrix can be found at XX. An examination of `r base_model_name` using the old ageing error matrix did not have a
...
illustrative sensitivities in section XX. We also present the previously accepted
...
The survey catchability parameter $q$ was fixed at 1.0, as for previous
 `r species` assessments. We explore a sensitivity run where survey
catchability is allowed to be calculated analytically or estimated in section XX.
