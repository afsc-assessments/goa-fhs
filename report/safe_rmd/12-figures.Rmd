...
(ref:NAAcap) Numbers at age (grey bubbles) by year for females (left) and males (right). Red line indicates mean age through time 

(ref:NALcap) Numbers at length (grey bubbles) by year for females (left) and males (right). Red line indicates mean length through time 

```{r NAA1, fig.cap="(ref:NAAcap) (1 of 2).", out.width = "50%",  eval = T}
knitr::include_graphics(here('model_runs','m0_8-newMI-biasAdj','plots','numbers1_sex1_beg.png'))
```

```{r NAA2, fig.cap="(ref:NAAcap) (2 of 2).", out.width = "50%",  eval = T}
knitr::include_graphics(here('model_runs','m0_8-newMI-biasAdj','plots','numbers1_sex2_beg.png'))
```

```{r NAL1, fig.cap="(ref:NALcap) (1 of 2).",  out.width = "50%", eval = T}
knitr::include_graphics(here('model_runs','m0_8-newMI-biasAdj','plots','numbers6_len_sex2.png'))
```


```{r NAL2, fig.cap="(ref:NALcap) (2 of 2).",  out.width = "50%", eval = T}
knitr::include_graphics(here('model_runs','m0_8-newMI-biasAdj','plots','numbers6_len_sex2.png'))
```
...
knitr::include_graphics(here::here('model_runs','m0_8-newMI-biasAdj','plots','bias_adj_panel.png'))
```


(ref:SSBcap2) Spawning stock biomass relative to B~35%~ and fishing mortality (F) relative to  F~35%~ from 1978-2021 (solid green line), the OFL control rule (solid red line), the $max_{ABC}$ control rule (dotted red line), B~35%~ (vertical grey line), and F~35%~ (horizontal grey line). The 2022 and 2023 spawning biomass and fishing mortality rates are as predicted by Alternatives 1 and 2 in the harvest projections.
...
```{r profileqm, fig.cap="3-d visualization of simultaneous profile over natural mortality and survey catchability. Natural mortality was identical for both sexes in these runs. The white contours indicate a log-likelihood difference of 1.92 from Model 17.1a; models within these bounds are statistically indistinguishable. The minimum likelihood occurred at q = 1.64 and M = 0.27.", eval = T}
knitr::include_graphics(here('model_runs','m0_8-newMI-biasAdj','profiles','profile_mq','mq_3d.png'))
```
...
```{r dietadult, echo = FALSE,  fig.cap="Diet composition for Gulf of Alaska adult flathead sole from the GOA ecosystem model (Aydin et al., 2007).", out.width="25%",eval = T, echo = FALSE}
knitr::include_graphics(here('figs','null.png')) 
```


```{r dietjuv, echo = FALSE,  fig.cap="Diet composition for Gulf of Alaska juvenile flathead sole from the GOA ecosystem model (Aydin et al., 2007).", out.width="25%",eval = T, echo = FALSE}
knitr::include_graphics(here('figs','null.png')) 
```

```{r mortadult, echo = FALSE,  fig.cap="Decomposition of natural mortality for Gulf of Alaska adult flathead sole from the GOA ecosystem model (Aydin et al., 2007).", out.width="25%",eval = T, echo = FALSE}
knitr::include_graphics(here('figs','null.png')) 
```


```{r mortjub, echo = FALSE,  fig.cap="Decomposition of natural mortality for Gulf of Alaska juvenile flathead sole from the GOA ecosystem model (Aydin et al., 2007).", out.width="25%",eval = T, echo = FALSE}
knitr::include_graphics(here('figs','null.png')) 
```