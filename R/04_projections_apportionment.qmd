...
read_rwout <- function(x){
 ...
 rwout <- rwout0[!grepl('2019|2021',rwout0 )] ## grab new runs (not 2019 nor 2021)
...
abc23 <- as.numeric( 40175)*apportionment
abc24 <- as.numeric(40046)*apportionment
...
# 
# index <- read.csv(here('data','survey',paste0(pull_date,'-index.csv'))) %>%
#   group_by(YEAR) %>%
#   summarise(BIOM_MT = BIOM,
#             CV_total=sqrt(BIOMVAR)/BIOM)
# 
...
props <- output$proportion_biomass_by_strata %>% filter(year == this_year) %>%   mutate(WestYakutat = Eastern*egfrac$Western.Fraction,
         Southeast = Eastern*egfrac$Eastern.Fraction) %>%
  select(Western, Central, WestYakutat,Southeast)
...
abc23 <- as.numeric(40175)*props
abc24 <- as.numeric(40046)*props



apportionment2 <- apply(props, 2, FUN = function(x) round(x*c(40175,40046) )) %>%
  rbind( round(props*100,2) ,.) %>%
  data.frame() %>%
  mutate(Total = c("",40175,40046),
