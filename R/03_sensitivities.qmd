...
r4ss::SS_profile(dir =  "C:/Users/maia.kapur/Work/assessments/2022/goa-fhs-2022/model_runs/m0_8-newMI-biasAdj/profiles/profile_mm",   
                 string = c("NatM_uniform_Fem_GP_1","NatM_uniform_Mal_GP_1"),
                 profilevec = mm.df)

mm.df$variable <- paste0('replist',1:100)
...
qm.df.plotme <- reshape2::melt(profile_mq_summary$likelihoods, id = 'Label') %>%
  ...
  filter( Label == 'TOTAL') %>%
  select(value,LnQ_base_Survey,
         NatM_uniform_Fem_GP_1 ) %>%
  tidyr::pivot_wider(names_from = NatM_uniform_Fem_GP_1 , values_from = value) %>%
  select(-LnQ_base_Survey) %>%
  as.matrix()

with(qm.df.plotme,persp::persp(x = LnQ_base_Survey , y =NatM_uniform_Fem_GP_1  , z = value))
...
require(plotly)
  ...
  plot_ly(  z = ~qm.df.plotme, type = "surface") %>%
    add_surface()
