...
ggplot(data = NULL, aes(x = yr, y = catch_mt)) +
  ...
  geom_point(data = catchfull, color = 'grey22', size = 5) +
  geom_point(data = subset(catchfull, yr > 2021), color = 'dodgerblue2',size = 5) +
...
p3 <- ggplot(data = NULL, aes(x = Yr, y = value2, color = SRC, fill = SRC)) +
  ...
  # theme(legend.position = 'bottomleft')+
    theme_void()+ theme(plot.background = element_rect(fill = "#212122"))+
      geom_errorbar(data = subset(survdat, variable == 'Obs' & SRC == 2022), aes(ymin = lwr, ymax = upr), width =0, color = 'grey77') +
  geom_point(data = subset(survdat, variable == 'Obs' & SRC == 2022), color = 'grey77', cex = 5) +
